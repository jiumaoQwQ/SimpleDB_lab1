...
public class CatalogTest extends SimpleDbTestBase {
     ...
     * I.e., if a name conflict exists, use the last table to be added 
    ...
    /**
     * Check that duplicate file ids are handled correctly (an id should refer to only one table)
     * So after adding a new file with a new name using an existing id,
     * that id should now reflect the new file and new name
     */
    @Test public void handleDuplicateIds() throws Exception {
    // create a file with a new name, using an existing id    
	String newName = SystemTestUtil.getUUID();
	DbFile f = new SkeletonFile(id2, Utility.getTupleDesc(2));

    // add the new file to the catalog and check that lookups by id
    // yield the new name and file
    ...
    // now do the reverse: add a new file with the original name for same id;
    // lookups by the id should yield the original name and new file
	DbFile f2 = new SkeletonFile(id2, Utility.getTupleDesc(2));
	Database.getCatalog().addTable(f2, name);
	assertEquals(name, Database.getCatalog().getTableName(id2));
	assertEquals(f2, Database.getCatalog().getDatabaseFile(id2));
    }
